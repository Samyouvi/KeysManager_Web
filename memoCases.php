...
class memoCases
{
  ...
  // Cas 2
  public function getUserWithRoomName($room_name) {
    echo '// Cas 2 //<br/>';
    $id_room = $this->_roomDAO->getIdWithName($room_name);
    if($id_room != -1) {
      echo '$id_room='.$id_room.'<br/>';
      ...
      if($id_lock != -1) {
        ...
        $id_keys = $this->_ouvreDAO->getIdsWithIdLock($id_lock);
        if ($id_keys != -1) {
          echo '$id_keys='.$id_keys.'<br/>';
          $id_keychains = $this->_keychainDAO->getIdWithKeyId($id_keys);
          if ($id_keychains != -1){
            echo '$id_keychains='.$id_keychains.'<br/>';
            $id_user = $this->_userDAO->getIdWithKeychainId($id_keychains);
            foreach ($id_user as $key) {
              $user_infos = $this->_userDAO->getInfosWithId($key->getName());
              echo '<pre>';
              print_r($user_infos);
              echo '</pre>';
          }
